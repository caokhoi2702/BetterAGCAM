...
# from BetterAGCAM.Methods.AGCAM.ViT_for_AGCAM.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD
from Methods.AGCAM.ViT_for_AGCAM.data import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD, IMAGENET_INCEPTION_MEAN, IMAGENET_INCEPTION_STD
...
#class Attention is modified for our method.
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # Identity layers for pytorch hook
        self.forward_hook_before_softmax = nn.Identity()
        self.backward_hook_after_softmax = nn.Identity()

    def forward(self, x):
        # replace a tensor with one that does not require gradient by pytorch detach() method
        # By doing so, our method ignore the gradient propagated through this layer so that dE_1^(k+1)/dE_2^k = I
        x=x.detach()
        B, N, C = x.shape
        ...
        # modified: forward hook
        attn = self.forward_hook_before_softmax(attn)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        # modified: backward hook
